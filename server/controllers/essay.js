...
const controllers = {
  async queryArticles (req, res, next) {
    ...
    try {
      const totalCount = await essayArticleModel.count()
  ...
  createArticle (req, res, next) {
    ...
    if (title && content) {
      let article = new essayArticleModel ({title, abstract, content})
  ...
  modifyArticle (req, res, next) {
    ...
    essayArticleModel.update({_id: id}, {title, abstract, content, image}, (err, article) => {
      if(err){
        console.log (err)
        res.status(400).send('Bad Request')
      }
