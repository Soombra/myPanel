<template>
  <div class="travel">
    <div class="list-page-head">
      <h3>游记列表</h3>
      <router-link class="option-btn" to="/travel/article/new">创建新文章</router-link>
    </div>
    <el-table
        :data="list"
      ...
      <el-table-column
          label="操作">
        <template slot-scope="scope">
          <router-link target='_blank' :to="`/travel/article/${scope.row._id}`" class="option-btn">编辑</router-link>
  ...
  export default {
    ...
    methods: {
      ...
      publishArticle (article) {
        travel.publishArticle(article._id).then(() => {
      ...
      unPublishArticle (article) {
        travel.unPublishArticle(article._id).then(() => {
      ...
      deleteArticle (article) {
        travel.deleteArticle(article._id).then(() => {
