<template>
  <div class="essay">
    <div class="list-page-head">
      <h3>生活随笔列表</h3>
      <router-link class="option-btn" to="/essay/article/new">创建新文章</router-link>
    </div>
    <el-table
        :data="list"
      ...
      <el-table-column
          label="操作">
        <template slot-scope="scope">
          <router-link target='_blank' :to="`/essay/article/${scope.row._id}`" class="option-btn">编辑</router-link>
  ...
  export default {
    ...
    methods: {
      ...
      publishArticle (article) {
        essay.publishArticle(article._id).then(() => {
      ...
      unPublishArticle (article) {
        essay.unPublishArticle(article._id).then(() => {
      ...
      deleteArticle (article) {
        essay.deleteArticle(article._id).then(() => {
